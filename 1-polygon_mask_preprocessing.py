...
for one_image_path, one_json_path, one_mask_output_name in tqdm(zip(images_path, mask_json_path, output_mask_name), total = images_path.__len__()):
    ...
    all_mask = (all_mask[:, :, np.newaxis] * 255).astype(np.uint8)

    # Step7. 將圖片、Mask 輸出
    one_image = cv2.cvtColor(one_image, cv2.COLOR_RGB2BGR)
    cv2.imwrite("Train_images/" + one_image_path.split("/")[-1], one_image.transpose((1, 0, 2)))
    cv2.imwrite(f"Train_Mask/{one_mask_output_name}", all_mask.transpose((1, 0, 2)))
